...
def update(execution_id, values, **kwargs):
    ...
    if values.get("status"):
        ...
        if test_execution.status.weight != 0 and not all_executions.filter(status__weight=0).exists():
    ...
    result["assignee__username"] = test_execution.assignee.username if test_execution.assignee else None
    result["tested_by__username"] = test_execution.tested_by.username if test_execution.tested_by else None
...
def add_link(values, update_tracker=False, **kwargs):
    ...
    if (
        link.is_defect and tracker is not None and update_tracker and not tracker.is_adding_testcase_to_issue_disabled()
