...
@permissions_required(("testruns.view_testexecution", "linkreference.add_linkreference"))
@rpc_method(name="Bug.report")
def report(execution_id, tracker_id, extra_dict=None, **kwargs):
    """
    .. function:: RPC Bug.report(execution_id, tracker_id)
        ...
        :type tracker_id: int
        :params extra_dict: for extra values
    ...
    if not bool(extra_dict):
        extra_dict = {}
    extra_dict.update({"user": request.user})

    if not tracker.is_adding_testcase_to_issue_disabled():
        url = tracker.report_issue_from_testexecution(execution, extra_dict)
...
@http_basic_auth_login_required
@rpc_method(name="Bug.bug_tracker_type")
def bug_tracker_type(tracker_id):
    bug_system = BugSystem.objects.get(pk=tracker_id)
    response = {"rc": 0, "response": bug_system.tracker_type}
    return response
