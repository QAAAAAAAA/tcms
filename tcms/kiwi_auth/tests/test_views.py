...
class TestForceToSetRandomKey(TestCase):
    ...
    def test_set_random_key_forcely(self):
        new_activation_key = UserActivationKey.set_random_key_for_user(self.new_user, force=True)
        self.assertEqual(self.origin_activation_key.user, new_activation_key.user)
        self.assertNotEqual(self.origin_activation_key.activation_key, new_activation_key.activation_key)
...
class TestLogout(TestCase):
    ...
    def test_logout_redirects_to_login_page(self):
        self.client.login(username=self.tester.username, password="password")  # nosec:B106:hardcoded_password_funcarg
...
Go to %(user_url)s to activate the account!"""
    ...
    def test_register_user_by_email_confirmation(self, send_mail):
        ...
        expected_subject = settings.EMAIL_SUBJECT_PREFIX + _("Your new %s account confirmation") % site.domain
        expected_body = (
            _(
                """Welcome %(user)s,
...
%(confirm_url)s"""
    ...
    def test_register_user_and_activate_by_admin(self):
        ...
        for (name, email) in settings.ADMINS:
            self.assertContains(response, '<a href="mailto:{}">{}</a>'.format(email, name), html=True)
    ...
    def test_only_one_superuser(self):
        user1 = User.objects.create_user("kiwi-tester", "tester@example.com", "password")
...
class TestConfirm(TestCase):
    ...
    def test_fail_if_activation_key_does_not_exist(self):
        ...
        self.assertContains(response, _("This activation key no longer exists in the database"))
