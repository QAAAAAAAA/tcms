...
class JIRA(IssueTrackerType):
    ...
    def is_adding_testcase_to_issue_disabled(self):
        return not (self.bug_system.base_url and self.bug_system.api_username and self.bug_system.api_password)
    ...
    def details(self, url):
        try:
            ...
            info = OrderedDict(
                {
                    "status": issue.fields.status.name if bool(issue.fields.status) else "None",
                    "reporter": issue.fields.reporter.name if bool(issue.fields.reporter) else "None",
                    "assignee": issue.fields.assignee.name if bool(issue.fields.assignee) else "None",
                    "priority": issue.fields.priority.name if bool(issue.fields.priority) else "None",
                    "updated": issue.fields.updated if bool(issue.fields.updated) else "None",
                }
            )

            return {
                "title": issue.fields.summary,
                "description": " | ".join("%s: %s" % (k, v) for k, v in info.items()),
            }
        except jira.exceptions.JIRAError:
            return super().details(url)

    def report_issue_from_testexecution(self, execution, extra_dict):
        """
        ...
        #     project = self.rpc.projects()[0]
        try:
            project = self.rpc.project(extra_dict["project_name"])
        except jira.exceptions.JIRAError:
            project = self.rpc.projects()[0]
        ...
        reporter = str(extra_dict["user"].email) if "@" in str(extra_dict["user"].email) else "sunh@sea.com"

        issue_kwargs = {
            "project": project.id,
            "summary": "Failed test: %s" % execution.case.summary,
            "description": (
                extra_dict["bugDescription"] if bool(extra_dict["bugDescription"]) else self._report_comment(execution)
            ),
            # "description": self._report_comment(execution),
            "issuetype": {"name": issue_type.name},
            "reporter": {"name": reporter},
            "labels": [i for i in extra_dict["labels"].split(",") if bool(i)],
            "priority": {"name": extra_dict["priority"]},
            "customfield_10203": {"value": "Always"},
            "customfield_10205": {"value": extra_dict["bugEnvironment"]},
            "customfield_10206": {"value": extra_dict["severity"]},
            "customfield_10207": {"value": extra_dict["bugType"]},
            "assignee": {"name": extra_dict["assignee"]},
        }

        if bool(extra_dict["epic_link"]):
            issue_kwargs.update({"customfield_10001": extra_dict["epic_link"]})

        new_issue = self.rpc.create_issue(**issue_kwargs)

        if bool(extra_dict["issue_link"]):
            self.rpc.create_issue_link(
                type=extra_dict["issue_link_type"], inwardIssue=new_issue.key, outwardIssue=extra_dict["issue_link"]
            )
...
class Gitlab(IssueTrackerType):
    ...
    def _rpc_connection(self):
        ...
        return gitlab.Gitlab(self.bug_system.api_url, private_token=self.bug_system.api_password)
