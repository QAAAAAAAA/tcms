...
class BugzillaThread(base.IntegrationThread):
    ...
    def post_comment(self):
        self.rpc.update_bugs(self.bug_id, {"comment": {"comment": self.text(), "is_private": False}})
...
class Bugzilla(base.IssueTrackerType):
    ...
    def __init__(self, bug_system, request):
        ...
        self._bugzilla_cache_dir = getattr(settings, "BUGZILLA_AUTH_CACHE_DIR", tempfile.mkdtemp(prefix=".bugzilla-"))
    ...
    def report_issue_from_testexecution(self, execution, extra_dict):
        ...
        try:
            new_bug_url = self.one_click_report(execution, extra_dict["user"], args)
