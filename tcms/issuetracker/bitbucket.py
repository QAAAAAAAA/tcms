...
class BitBucketAPI:
    ...
    def get_comments(self, issue_id):
        url = "{0}{1}{2}{3}".format(self.endpoint_url, "/issues/", issue_id, "/comments?sort=-updated_on")
        return self._request("GET", url, headers=self.headers, auth=self.auth)

    def delete_comment(self, issue_id, comment_id):
        url = "{0}{1}{2}{3}{4}".format(self.endpoint_url, "/issues/", issue_id, "/comments/", comment_id)
    ...
    def _construct_endpoint_url(api_version, url):
        ...
        endpoint_url = "{0}/{1}/{2}/{3}/{4}".format(base_url, api_version, "repositories", workspace, repository)
        return endpoint_url
