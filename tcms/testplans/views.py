...
@method_decorator(
    object_permission_required("testplans.change_testplan", (TestPlan, "pk", "pk"), accept_global_perms=True),
    name="dispatch",
)
class Edit(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["notify_formset"] = kwargs.get("notify_formset") or PlanNotifyFormSet(instance=self.object)
        return context
...
@method_decorator(
    object_permission_required("testplans.view_testplan", (TestPlan, "pk", "pk"), accept_global_perms=True),
    name="dispatch",
)
class TestPlanGetView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["test_runs"] = TestRun.objects.filter(plan_id=self.object.pk, stop_date__isnull=True).order_by("-id")[
            :5
        ]
        context["OBJECT_MENU_ITEMS"] = [
            (
                ...
                [
                    ...
                    (_("Mind map"), reverse("plans-mind-map", args=[self.object.pk])),
                ],
            )
        ]
...
class GetTestPlanRedirectView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return HttpResponsePermanentRedirect(reverse("test_plan_url", args=[test_plan.pk, slugify(test_plan.name)]))
...
class Clone(FormView):
    ...
    def form_valid(self, form):
        ...
        return HttpResponseRedirect(reverse("test_plan_url_short", args=[cloned_plan.pk]))
...
class Mindmap(DetailView):
    template_name = "testplans/mind_map.html"
    http_method_names = ["get"]
    model = TestPlan
