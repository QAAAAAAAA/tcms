...
class NewRunForm(forms.ModelForm):
    ...
    def populate(self, plan_id):
        ...
        self.fields["case"].queryset = TestCase.objects.filter(case_status__is_confirmed=True).all()
...
class SearchRunForm(forms.ModelForm):
    ...
    def populate(self, product_id=None):
        if product_id:
            self.fields["version"].queryset = Version.objects.filter(product__pk=product_id)
            # self.fields["build"].queryset = Build.objects.filter(version__product=product_id)
