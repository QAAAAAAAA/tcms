...
$(document).ready(() => {
  ...
  jsonRPC('TestExecutionStatus.filter', {}, executionStatuses => {
    ...
    jsonRPC('TestExecution.filter', rpcQuery, testExecutions => {
      ...
      drawBugPercentBar(testExecutions)
...
function drawBugPercentBar(testExecutions){
  let positiveCount = 0
  let negativeCount = 0
  let allCount = 0
  const ids = {}
...
function renderProgressBars(renderType, positiveCount, negativeCount, allCount) {
  if(renderType === 'bug'){
...
function fileBugFromExecution(execution) {
  ...
  // Shopee Jira Bug Ticket
  loadJiraReportForm()
  $('.one-click-bug-report-form #id-issue-tracker').change(() => loadJiraReportForm())
  ...
  $('.one-click-bug-report-form').submit(() => {
    ...
    const jiraProjectName = $('.one-click-bug-report-form #jira-project-name-input').val()
    const jiraAssignee = $('.one-click-bug-report-form #jira-assignee-input').val()
    const jiraPriority = $('.one-click-bug-report-form #jira-priority-field').val()
    const jiraBugType = $('.one-click-bug-report-form #jira-bug-type-field').val()
    const jiraSeverity = $('.one-click-bug-report-form #jira-severity-field').val()
    const jiraBugEnvironment = $('.one-click-bug-report-form #jira-server-environment-field').val()
    const jiraLabels = $('.one-click-bug-report-form #jira-labels-input').val()
    const jiraEpicLink = $('.one-click-bug-report-form #jira-epic-link-input').val()
    const jiraIssueLinkType = $('.one-click-bug-report-form #jira-issuelinks-linktype').val()
    const jiraIssueLink = $('.one-click-bug-report-form #jira-issue-link-input').val()
    const jiraBugDescription = $('.one-click-bug-report-form #jira-description-field').val()

    jsonRPC('Bug.report', [execution.id, trackerId, { project_name: jiraProjectName, assignee: jiraAssignee, priority: jiraPriority, bugType: jiraBugType, severity: jiraSeverity, bugEnvironment: jiraBugEnvironment, labels: jiraLabels, epic_link: jiraEpicLink, issue_link_type: jiraIssueLinkType, issue_link: jiraIssueLink, bugDescription: jiraBugDescription }], result => {
...
function loadJiraReportForm(){
    var id_issue_tracker = $('.one-click-bug-report-form #id-issue-tracker').val()
    jsonRPC('Bug.bug_tracker_type', [id_issue_tracker], result => {
        if (result.response.includes('JIRA')){
          $('.one-click-bug-report-form #jira-project-name').show()
          $('.one-click-bug-report-form #jira-assign-to').show()
          $('.one-click-bug-report-form #jira-priority').show()
          $('.one-click-bug-report-form #jira-bug-type').show()
          $('.one-click-bug-report-form #jira-severity').show()
          $('.one-click-bug-report-form #jira-server-environment').show()
          $('.one-click-bug-report-form #jira-labels').show()
          $('.one-click-bug-report-form #jira-epic-link').show()
          $('.one-click-bug-report-form #jira-link-issue').show()
          $('.one-click-bug-report-form #jira-description').show()
        }
    })
}