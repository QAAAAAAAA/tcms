...
@method_decorator(
    object_permission_required("testruns.view_testrun", (TestRun, "pk", "pk"), accept_global_perms=True),
    name="dispatch",
)
        ...
        context["OBJECT_MENU_ITEMS"] = [
            (
                ...
                [
                    ...
                    (_("Mind map"), reverse("testruns-mind-map", args=[self.object.pk])),
                ],
            )
        ]
...
@method_decorator(
    object_permission_required("testruns.change_testrun", (TestRun, "pk", "pk"), accept_global_perms=True),
    name="dispatch",
)
...
class CloneTestRunView(NewTestRunView):
    ...
    def get(self, request, pk):  # pylint: disable=arguments-differ
        ...
        request.GET.setlist("c", test_run.executions.all().values_list("case", flat=True))
...
class Mindmap(DetailView):
    template_name = "testruns/mind_map.html"
    http_method_names = ["get"]
    model = TestRun
