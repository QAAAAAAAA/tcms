...
class TestGetRun(BaseCaseRun):
    ...
    def test_get_a_run(self):
        ...
        self.assertContains(
            response,
            '<h2 class="card-pf-title"><span class="fa fa-tags"></span>{0}</h2>'.format(_("Tags")),
            html=True,
        )
...
class TestCreateNewRun(BasePlanCase):
    ...
    def test_post_creates_new_run(self):
        ...
        for case, execution in zip((self.case_1, self.case_2), new_run.executions.order_by("case")):
...
class TestCloneRunView(PermissionsTestCase):
    ...
    def verify_get_with_permission(self):
        ...
        for execution in (self.execution_1, self.execution_2):
            ...
            self.assertContains(
                response,
                '<a href="%s">TC-%d: %s</a>' % (case_url, execution.case.pk, execution.case.summary),
                html=True,
            )
...
class TestSearchRuns(BaseCaseRun):
    ...
    def test_search_page_is_shown_with_get_parameter_used(self):
        ...
        self.assertContains(
            response,
            '<option value="%d" selected>%s</option>' % (self.product.pk, self.product.name),
            html=True,
        )
...
class TestRunStatusMenu(BaseCaseRun):
    ...
    def test_get_status_options_with_permission(self):
        ...
        for execution_status in TestExecutionStatus.objects.all():
            self.assertContains(
                response,
                '<span class="{0}"></span>{1}'.format(execution_status.icon, execution_status.name),
                html=True,
            )
